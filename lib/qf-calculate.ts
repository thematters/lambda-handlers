...
const billboardUrl = `https://billboard.matters-lab.io/`
  ...
  const aggPerProjFrom = d3.rollup(
    seqs.filter(
      ...
      ({ from, to, dataHash }) =>
        ...
        sendersOut.get(from.toLowerCase()).trustPoints > 0 // && from address is not too low trust-score
    ),
    (g) => ({
  ...
  const authorGroups = d3.rollup(
    ...
    (groups) =>
      +(
        ...
        (
          Number(groups.reduce((acc, d) => acc + BigInt(d.clr_amount), 0n)) /
          1e6
        ).toFixed(2)
      ),
    (d) => d.userName
...
-- ORDER BY u.id, crypto_wallet.updated_at DESC NULLS LAST ; `
  ...
  const items = authors.map(
    ({ id, userName, displayName, email, language, walletChanges }) => ({
      ...
      amount: authorGroups.get(userName)?.toFixed(2),
    })
  )
...
RETURNING * ; `
  ...
  return retDoUpdateLastNotified // items // authors // sent
}
...
async function sendQfNotifInsite(
  ...
  const allNotices = items.map(
    ({ userId, language, amount, walletChanges, ...rest }) => ({
      userId, // language,
      message: getNoticeMessage(language, amount as string, walletChanges),
    })
  )
  ...
  if (messageIdsMap.size < allMessages.length) {
    ...
    ${sql.array(
      missingOnes.map(() =>
        JSON.stringify({ link: `https://billboard.matters-lab.io/claim` })
            ...
            personalizations: [
              {
                ...
                dynamicTemplateData: {
                  ...
                  amount,
                  walletChanges,
                  claimLink: `https://billboard.matters-lab.io/claim`,
                  billboardUrl,
                },
              },
            ],
