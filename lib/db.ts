...
SELECT draft.id, article_id, draft.title, article.slug, draft.summary, draft.content, draft.created_at, -- article.state, draft.publish_state,
  t.*
FROM draft JOIN article ON article_id=article.id
LEFT JOIN (
...
WHERE state='active' AND publish_state='published'
  ...
  AND article_id IN (
    ...
    ${allRecentArticles}
    UNION ALL
    SELECT ${articleId ?? null}
  )
  -- AND age(draft.created_at) <= interval '1 year'
  -- ORDER BY RANDOM() LIMIT 3 ;
ORDER BY ${
      articleId ? sql` (article_id=${articleId}) DESC, ` : sql``
